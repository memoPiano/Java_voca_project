...
public class FileManager {
    ...
    // 예문 파일 → Map 로딩
    public static HashMap<String, String> loadExamples(String path) {
        ...
        try (Scanner file = new Scanner(new File(path))) {
            while (file.hasNext()) {
                ...
                String eng = temp[0].trim();
                String sentence = temp[1].trim();
                map.put(eng, sentence);
            }
        } catch (FileNotFoundException e) {
            // 예문파일 없으면 그냥 빈 map 반환
        }
        return map;
    }

    // Map → 예문 파일 저장
    public static void saveExamples(String path, Map<String, String> map) {
